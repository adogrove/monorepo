...
import type { Resolver, UserResolver } from '../src/types.js'
...
export async function setupApp() {
  const { context, options } = getActiveTestOrFail()

  const { fs } = context
  await fs.mkdir(fs.basePath, { recursive: true })

  const filename = stringHelpers.slug(options.title)
        ...
        database: defineLucidConfig({
          ...
          connections: {
            sqlite: {
              ...
              connection: {
                filename: join(fs.basePath, `db-${filename}.sqlite3`),
                debug: true,
              },
